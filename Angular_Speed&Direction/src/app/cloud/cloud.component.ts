...
export class CloudComponent implements OnInit {
  ...
  wmsCloud = L.tileLayer.wms('http://localhost:8080/ncWMS2/wms', {
    layers: '1/hcc',
    transparent: false,
    format: 'image/png',
    opacity: 0.4,
  ...
  ngOnInit(): void {
    ...
    this.map.addLayer(this.wmsCloud)
    this.map.addLayer(this.osm);

    this.map.on('click', this.GetFeatureInfoWMS, this.wmsCloud);
    ...
    fetch('http://localhost:8080/ncWMS2/wms?SERVICE=WMS&REQUEST=GetCapabilities&VERSION=1.3.0').then(res => res.text()).then((res) => {
      ...
      document.getElementById('getGraph').addEventListener('click', () => {
        ...
        if (startD == this.cusStartD && endD == this.cusEndD && startT < this.cusStartT && endT >= this.cusEndT) {
          //var Graph = `http://localhost:8080/ncWMS2/wms?REQUEST=GetTimeseries&LAYERS=1/hcc&QUERY_LAYERS=1/hcc&BBOX=-${obj.bbox}&SRS=CRS:84&FEATURE_COUNT=5&HEIGHT=${obj.height}&WIDTH=${obj.width}&X=${Math.round(xy.x)}&Y=${Math.round(xy.y)}&STYLES=default/default&VERSION=1.1.1&TIME=${this.cusStartD}T${this.cusStartT}.000Z/${this.cusEndD}T${this.cusEndT}.000Z&INFO_FORMAT=image/png`;

          var Graph = 'http://localhost:8080/ncWMS2/wms?REQUEST=GetTimeseries&LAYERS=1/hcc&QUERY_LAYERS=1/hcc&BBOX=-180,-144,180,144&SRS=CRS:84&FEATURE_COUNT=5&HEIGHT=600&WIDTH=750&X=490&Y=163&STYLES=default/default&VERSION=1.1.1&TIME=2023-02-18T00:00:00.000Z/2023-02-18T14:00:00.000Z&INFO_FORMAT=image/png';
