...
export class HumidityComponent implements OnInit {
  ...
  wmsHumidity = L.tileLayer.wms('http://localhost:8080/ncWMS2/wms', {
    layers: '4/r',
    transparent: false,
    format: 'image/png',
    opacity: 0.4,
  ...
  ngOnInit(): void {
    ...
    this.map.addLayer(this.wmsHumidity)
    this.map.addLayer(this.googleSat);

    this.map.on('click', this.GetFeatureInfoWMS, this.wmsHumidity);
    ...
    fetch('http://localhost:8080/ncWMS2/wms?SERVICE=WMS&REQUEST=GetCapabilities&VERSION=1.3.0').then(res => res.text()).then((res) => {
      ...
      document.getElementById('getGraph').addEventListener('click', () => {
        ...
        if (startD == this.cusStartD && endD == this.cusEndD && startT < this.cusStartT && endT >= this.cusEndT) {
          // var Graph = `http://localhost:8080/ncWMS2/wms?REQUEST=GetTimeseries&LAYERS=4/r&QUERY_LAYERS=4/r&BBOX=-${obj.bbox}&SRS=CRS:84&FEATURE_COUNT=5&HEIGHT=${obj.height}&WIDTH=${obj.width}&X=${Math.round(xy.x)}&Y=${Math.round(xy.y)}&STYLES=default/default&VERSION=1.1.1&TIME=${this.cusStartD}T${this.cusStartT}.000Z/${this.cusEndD}T${this.cusEndT}.000Z&INFO_FORMAT=image/png`;

          var Graph = 'http://localhost:8080/ncWMS2/wms?REQUEST=GetTimeseries&LAYERS=4/r&QUERY_LAYERS=4/r&BBOX=-180,-144,180,144&SRS=CRS:84&FEATURE_COUNT=5&HEIGHT=600&WIDTH=750&X=203&Y=258&STYLES=default/default&ELEVATION=1&VERSION=1.1.1&TIME=2023-02-17T00:00:00.000Z/2023-02-18T08:00:00.000Z&INFO_FORMAT=image/png';  
